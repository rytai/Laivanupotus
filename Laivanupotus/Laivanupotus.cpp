...
public:
	...
	void WriteHighScoresToFile(string new_name = "", int new_score = 0) {
		...
		int smallestIndex;
		// uudet pisteet pienimm‰n tilalle
		if (new_score != 0) {
...
private:
	...
	//T‰ll‰ kerrotaan kerroin kun hit streak toteutuu.
...
public:
	...
	void Miss() {
		//Annetaan pisteet, jotka on ker‰tty, jos on.
		score += (comboScore *currentMultiplier);
		//Nollataa kerroin ja ker‰tyt pisteet.
		currentMultiplier	= 1;
		comboScore			= 0;	
	}
};
